...
-->
  <div>
    ...
    <#if postalAddress.stateProvinceGeoId?has_content && postalAddress.stateProvinceGeoId != "_NA_">
      ...
      ${stateProvince.geoName} (${stateProvince.abbreviation?default(stateProvince.geoId)}),
    </#if>
    <#if postalAddress.countryGeoId?has_content>
      <#assign country = postalAddress.getRelatedOne("CountryGeo", true)>
      ${country.get("geoName", locale)?default(country.geoId)} (${country.abbreviation?default(country.geoId)})
    </#if>
    </div>
    <#if !postalAddress.countryGeoId?has_content>
