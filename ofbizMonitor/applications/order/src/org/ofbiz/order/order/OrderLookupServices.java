...
import org.ofbiz.entity.condition.EntityExpr; import org.ofbiz.entity.condition.EntityFunction;
...
public class OrderLookupServices {
    ...
    public static Map<String, Object> findOrders(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        String queryCond = (String)context.get("queryCond");
        String queryValue = (String)context.get("queryValue");
        ...
        // start the lookup
        String orderId = null; /*String orderId = (String) context.get("orderId");*/if("orderId".equals(queryCond)){orderId = queryValue;}
        ...
        //bellyanna - add email address and ebay user ID to the view -- START
        String contactMechTypeIdEmail = null;
        if("contactMechTypeIdEmail".equals(queryCond)){
            contactMechTypeIdEmail = queryValue;
        }
        
        String contactMechTypeIdEbay = null;
        if("contactMechTypeIdEbay".equals(queryCond)){
            contactMechTypeIdEbay = queryValue;
            }
        //bellyanna - add email address and ebay user ID to the view -- END

        // add the role data to the view
        if (roleTypeList != null || partyId != null || contactMechTypeIdEbay != null || contactMechTypeIdEmail != null) {
        ...
        boolean isIncludeOrderItemTable = false;
        ...
        //Bellyanna - email and ebay user id -- START
        if(contactMechTypeIdEmail != null || contactMechTypeIdEbay != null){
            dve.addMemberEntity("PCM", "PartyContactMech");
            dve.addAlias("PCM", "contactMechId");
            dve.addViewLink("OT", "PCM", Boolean.FALSE, UtilMisc.toList(new ModelKeyMap("partyId", "partyId")));
            
            dve.addMemberEntity("CM", "ContactMech");
            dve.addAlias("CM", "contactMechTypeId");
            dve.addAlias("CM", "infoString");
            dve.addViewLink("PCM", "CM", Boolean.FALSE, UtilMisc.toList(new ModelKeyMap("contactMechId", "contactMechId")));
        }
        
        if(UtilValidate.isNotEmpty(contactMechTypeIdEmail)){
            List andExpr = FastList.newInstance();
            andExpr.add(EntityCondition.makeCondition("contactMechTypeId", EntityOperator.EQUALS, "EMAIL_ADDRESS"));
            andExpr.add(EntityCondition.makeCondition(EntityFunction.UPPER_FIELD("infoString"), EntityOperator.EQUALS, contactMechTypeIdEmail.toUpperCase()));
            conditions.add(EntityCondition.makeCondition(andExpr, EntityOperator.AND));
        }
        
        if(UtilValidate.isNotEmpty(contactMechTypeIdEbay)){
            List andExpr = FastList.newInstance();
            andExpr.add(EntityCondition.makeCondition("contactMechTypeId", EntityOperator.EQUALS, "EBAY_USER_ID"));
            andExpr.add(EntityCondition.makeCondition("infoString", EntityOperator.EQUALS, contactMechTypeIdEbay));
            conditions.add(EntityCondition.makeCondition(andExpr, EntityOperator.AND));
        }
        //Bellyanna - email and ebay user id -- END
        ...
        //Bellyanna - tracking code -- START
        String trackingCode = null;
        if("trackingCode".equals(queryCond)){
            trackingCode = queryValue;
        }
        
        if (UtilValidate.isNotEmpty(trackingCode)) {
            dve.addMemberEntity("SH", "Shipment");
            dve.addAlias("SH", "shipmentId");
            dve.addViewLink("OH", "SH", Boolean.FALSE, UtilMisc.toList(new ModelKeyMap("orderId", "primaryOrderId")));
            
            dve.addMemberEntity("SHPKRTSG", "ShipmentPackageRouteSeg");
            dve.addAlias("SHPKRTSG", "trackingCode");
            dve.addViewLink("SH", "SHPKRTSG", Boolean.FALSE, UtilMisc.toList(new ModelKeyMap("shipmentId", "shipmentId")));
            
            paramList.add("trackingCode=" + trackingCode);
            conditions.add(makeExpr("trackingCode", trackingCode));
        }
        //Bellyanna - tracking code -- END
        ...
        //Bellyanna - eBay Item Number
        if("ebayItemNumber".equals(queryCond)){
            if(UtilValidate.isNotEmpty(queryValue)){
                if(!isIncludeOrderItemTable){
                    dve.addMemberEntity("OI", "OrderItem");
                    dve.addAlias("OI", "correspondingPoId");
                    dve.addAlias("OI", "subscriptionId");
                    dve.addAlias("OI", "productId");
                ...
                dve.addMemberEntity("OIA", "OrderItemAttribute");
                dve.addAlias("OIA", "attrName");
                dve.addAlias("OIA", "attrValue");
                dve.addViewLink("OI", "OIA", Boolean.FALSE, UtilMisc.toList(new ModelKeyMap("orderId", "orderId")));
                
                conditions.add(EntityCondition.makeCondition("attrName", EntityOperator.EQUALS, "eBay Item Number"));
                conditions.add(EntityCondition.makeCondition("attrValue", EntityOperator.EQUALS, queryValue));
            }
        }
        
        //Bellyanna - eBay Transaction ID
        if("ebayTransactionId".equals(queryCond)){
            if(UtilValidate.isNotEmpty(queryValue)){
                if(!isIncludeOrderItemTable){
                    dve.addMemberEntity("OI", "OrderItem");
                    dve.addAlias("OI", "correspondingPoId");
                ...
                conditions.add(EntityCondition.makeCondition("attrName", EntityOperator.EQUALS, "EBAY_TRAN_ID"));
                conditions.add(EntityCondition.makeCondition("attrValue", EntityOperator.EQUALS, queryValue));
            }
        }
        
        //Bellyanna - Promotion Code
        if("promoCode".equals(queryCond)){
            if(UtilValidate.isNotEmpty(queryValue)){
                dve.addMemberEntity("PP", "ProductPromoUse");
                dve.addAlias("PP", "productPromoCodeId");
                dve.addViewLink("OH", "PP", Boolean.FALSE, UtilMisc.toList(new ModelKeyMap("orderId", "orderId")));
                conditions.add(EntityCondition.makeCondition("productPromoCodeId", EntityOperator.EQUALS, queryValue));
            }
        }
        
        //Bellyanna - PayPalTransactionID
        if("paypalTransactionId".equals(queryCond)){
            if(UtilValidate.isNotEmpty(queryValue)){
                if (UtilValidate.isNotEmpty(gatewayAvsResult) || UtilValidate.isNotEmpty(gatewayScoreResult)) {
                    conditions.add(EntityCondition.makeCondition("referenceNum", EntityOperator.EQUALS, queryValue));
                }
                else {
                    dve.addMemberEntity("OPP", "OrderPaymentPreference");
                    dve.addMemberEntity("PGR", "PaymentGatewayResponse");
