...
public class CheckOutHelper {
    ...
    public Map<String, Object> createOrder(GenericValue userLogin, String distributorId, String affiliateId,
        ...
        String originOrderId = (String) this.cart.getAttribute("originOrderId"); Debug.logError("OriginOrderId is " + originOrderId, module);
        ...
        if (party != null) {
            ...
            //Bellyanna -- START
            try {
                List<GenericValue> orderPhoneContactMech = delegator.findByAnd("OrderContactMech", UtilMisc.toMap("orderId", originOrderId, "contactMechPurposeTypeId", "PRIMARY_PHONE"), null, false);
                Debug.logError("OriginOrderId is " + orderPhoneContactMech, module);
                if (orderPhoneContactMech != null) {
                    GenericValue orderPhone = EntityUtil.getFirst(orderPhoneContactMech);
                    Debug.logError("orderPhone contactMechId is " + orderPhone, module);
                    GenericValue newOrderPhone = this.delegator.makeValue("OrderContactMech",
                            UtilMisc.toMap("orderId", orderId, "contactMechId", orderPhone.getString("contactMechId"), "contactMechPurposeTypeId", "PRIMARY_PHONE"));
                    toBeStored.add(newOrderPhone);
                }
                List<GenericValue> ebayContactMech = delegator.findByAnd("OrderContactMech", UtilMisc.toMap("orderId", originOrderId, "contactMechPurposeTypeId", "EBAY_USER_ID"), null, false);
                if (ebayContactMech != null) {
                    GenericValue ebayUserId = EntityUtil.getFirst(ebayContactMech);
                    GenericValue newEbayUserId = this.delegator.makeValue("OrderContactMech",
                                                                          UtilMisc.toMap("orderId", orderId, "contactMechId", ebayUserId.getString("contactMechId"), "contactMechPurposeTypeId", "EBAY_USER_ID"));
                    toBeStored.add(newEbayUserId);
                }
            } catch (GenericEntityException e) {
                Debug.logWarning(e, "Bellyanna Code error getting Phone and eBay User Id contact mech", module);
            }
            //Bellyanna -- END
        }
