...
<div class="screenlet">
  ...
  <div class="screenlet-body">
     ...
     <#if orderHeader.salesChannelEnumId?has_content && orderHeader.salesChannelEnumId == "EBAY_SALES_CHANNEL">
     <tr>
       <td align="center" valign="top" width="29%" class="label">&nbsp;PayPal ${uiLabelMap.FormFieldTitle_transactionId}</td>
       <td width="1%">&nbsp;</td>
       <td width="60%">
	<#list orderPaymentPreferences as orderPaymentPreference>
	<#assign PaypalGatewayResponses = orderPaymentPreference.getRelated("PaymentGatewayResponse", null, null, false)>
	<#list PaypalGatewayResponses as PaypalGatewayResponse>
	  <#if PaypalGatewayResponse.referenceNum?has_content>
	  <#assign PaypalTransactionId = PaypalGatewayResponse.referenceNum>
	  ${PaypalTransactionId}</#if>
	</#list>
	</#list>
       </td>
       <td width="10%">&nbsp;</td>
     ...
     <#-- refund info - START -->
       <#assign refund = Static["org.ofbiz.entity.util.EntityUtil"].getFirst(delegator.findByAnd("Refund", Static["org.ofbiz.base.util.UtilMisc"].toMap("orderId" ,orderId), null, false))?if_exists>
      <tr><td colspan="4"><hr /></td></tr>
       <tr>
        <td width="29%" valign="top" align="center" class="label">&nbsp;Refund Entry</td>
        <td colspan="2">
        	<#if refund?has_content>
        		<#assign refundStatus = refund.getRelatedOne("StatusItem", false)?if_exists>

        		${refundStatus.get("description",locale)!} - ${refund.lastUpdatedStamp}
                  &nbsp;
                  ${uiLabelMap.CommonBy} - [${refund.createdByUserLogin}]<br/>
                Amount: ${refund.currencyUomId!} ${refund.amount!}<br/>
                <#if refund.reasonCode??>
                    ...
                    Reason: ${reasonDesc.get("description")}<br/>
                </#if>
        		Desc: ${refund.reason!}
        	 	<#if refundStatus.statusCode == 'COMPLETED'>
        	 		<br/>
        			Refund PayPal trans ID: ${refund.refundPaypalTransId!}
        		</#if>
        	<#else>No refund found!
        	</#if><br/>
        	<#if ! refundStatus?? || refundStatus.statusCode == 'PENDING'>
	          <a class="buttontext" href="/ordermgr/control/editOrderRefund?orderId=${orderId}&refundId=${refund.refundId!}" target="refund">
	          	<#if refund?has_content>Edit<#else>Create</#if>
	          </a>
            <#elseif ! refundStatus?? || refundStatus.statusCode == 'REJECTED'>
              <a class="buttontext" href="/ordermgr/control/editOrderRefund?orderId=${orderId}&refundId=${refund.refundId!}" target="refund">
	          	<#if refund?has_content>Recreate<#else>Create</#if>
	          </a>
            </#if>
        </td>
       </tr>
