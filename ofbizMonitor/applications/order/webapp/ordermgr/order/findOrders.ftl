...
<!-- //
function lookupOrders(click) { var queryCond = document.lookuporder.queryCond.value; var queryValue = document.lookuporder.queryValue.value;
    orderIdValue = document.lookuporder.orderId.value;
    if("orderId" == queryCond) {
      if (queryValue.length > 1) {
        document.lookuporder.orderId.value = queryValue;
...
<form name='lookupandhidefields${requestParameters.hideFields?default("Y")}' method="post" action="<@ofbizUrl>searchorders</@ofbizUrl>">
  ...
  <input type='hidden' name='queryValue' value='${requestParameters.queryValue?if_exists}'/>
  <input type='hidden' name='queryCond' value='${requestParameters.queryCond?if_exists}'/>
</form>
</#if>
      ...
      <table class="basic-table" cellspacing='0'>
        <tr>
          <td align='center' width='100%'>
            <table class="basic-table" cellspacing='0'>
              <tr>
                <td width='25%' align='right' class='label'>${uiLabelMap.CommonSearch}</td>
                <td width='5%'>&nbsp;</td>
                <td align='left'><input type='text' id='queryValue' name='queryValue' onchange='return trim(this)'/> ${uiLabelMap.CommonIn}
                  <select id="queryCond" name="queryCond">
                    <option value="orderId">${uiLabelMap.OrderOrderId}</option>
                    <option value="contactMechTypeIdEmail">${uiLabelMap.CommonEmail}</option>
                    <option value="contactMechTypeIdEbay">eBay ${uiLabelMap.CommonUserId}</option>
                    <option value="trackingCode">${uiLabelMap.OrderTrackingCode}</option>
                    <option value="ebayItemNumber">eBay Item Number</option>
                    <option value="ebayTransactionId">eBay Transaction ID</option>
                    <option value="paypalTransactionId">PayPal Transaction ID</option>
                    <option value="promoCode">${uiLabelMap.OrderPromotionCouponCodes}</option>
                  </select>
                </td>
              </tr>
             <#--<tr>
...
<script language="JavaScript" type="text/javascript">
<!--//
document.lookuporder.queryValue.focus();
//-->
</script>
</#if>
