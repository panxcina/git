  ...
  <div class="screenlet">
    <div class="screenlet-title-bar">
      <ul>
        ...
        <li class="collapsed"><a onclick="javascript:toggleScreenlet(this, 'ActionScreenletBody', 'true', '${uiLabelMap.CommonExpand}', '${uiLabelMap.CommonCollapse}');" title="Expand">&nbsp;</a></li>
      </ul>
      <br class="clear"/>
    </div>
    <div class="screenlet-body" id="ActionScreenletBody" style="display: none;">
      <ul>
      ...
      <table width="100%" border="0" cellpadding="1" cellspacing="0">
        ...
        <#if shipGroup.trackingNumber?has_content || orderShipmentInfoSummaryList?has_content>
          ...
          <tr>
            ...
            <td valign="top" width="80%">
              ...
              <#if shipGroup.trackingNumber?has_content>
                ...
                <#if orderHeader.statusId != "ORDER_COMPLETED">
                    <form name="cancelEpacketTrackingNumber" method="post" action="<@ofbizUrl>cancelEpacketTrackingNumber</@ofbizUrl>">
                        <input type="hidden" name="trackingNumber" value="${shipGroup.trackingNumber}"/>
                        <input type="hidden" name="productStoreId" value="${productStore.productStoreId}"/>
                    </form>
                    <a href="javascript:document.cancelEpacketTrackingNumber.submit()" class="buttontext">Cancel Tracking Number</a>
                    <#else>
                        <form name="recreateEpacketTrackingNumber" method="post" action="<@ofbizUrl>recreateEpacketTrackingNumber</@ofbizUrl>">
                            <input type="hidden" name="trackingNumber" value="${shipGroup.trackingNumber}"/>
                            <input type="hidden" name="productStoreId" value="${productStore.productStoreId}"/>
                        </form>
                        <a href="javascript:document.recreateEpacketTrackingNumber.submit()" class="buttontext">Recreate Tracking Number</a>
                </#if>
              </#if>
              <#if orderShipmentInfoSummaryList?has_content>
                <#list orderShipmentInfoSummaryList as orderShipmentInfoSummary>
                  <#if orderShipmentInfoSummary.shipGroupSeqId?if_exists == shipGroup.shipGroupSeqId?if_exists>
                    <div>
                    <#assign shipmentPackageWeight = delegator.findOne("ShipmentPackage", Static["org.ofbiz.base.util.UtilMisc"].toMap("shipmentId", orderShipmentInfoSummary.shipmentId, "shipmentPackageSeqId",orderShipmentInfoSummary.shipmentPackageSeqId), false)>
                      <#if (orderShipmentInfoSummaryList?size > 1)>${uiLabelMap.ProductPackage} ${orderShipmentInfoSummary.shipmentPackageSeqId}: </#if>
                      ${orderShipmentInfoSummary.trackingCode?default("[${uiLabelMap.OrderNotYetKnown}]")} <a href="http://www.17track.net/en/result/post.shtml?nums=${orderShipmentInfoSummary.trackingCode}" target="_blank" class="buttontext">Track</a>
                      <#if orderShipmentInfoSummary.boxNumber?has_content> ${uiLabelMap.ProductBox} #${orderShipmentInfoSummary.boxNumber}</#if>
                      &nbsp;||&nbsp;<span class="label">${uiLabelMap.CommonWeight}</span>
                      <#if shipmentPackageWeight.weight?has_content>${shipmentPackageWeight.weight?default("[${uiLabelMap.OrderNotYetKnown}]")}</#if>&nbsp;${uiLabelMap.ProductShipmentUomAbbreviation_WT_g}</#if>
                      <#if orderShipmentInfoSummary.packageTransportCost?has_content>&nbsp;||&nbsp;<span class="label">Cost</span>${orderShipmentInfoSummary.packageTransportCost} rmb</#if>
                      <#if shipmentMethodType.shipmentMethodTypeId == "EPACKET" || shipmentMethodType.shipmentMethodTypeId == "NO_SHIPPING">
                      <#assign trackingStatus = delegator.findOne("ShipmentPackageRouteSeg", Static["org.ofbiz.base.util.UtilMisc"].toMap("shipmentId", orderShipmentInfoSummary.shipmentId, "shipmentPackageSeqId",orderShipmentInfoSummary.shipmentPackageSeqId, "shipmentRouteSegmentId", orderShipmentInfoSummary.shipmentRouteSegmentId), false)>
                      <#if trackingStatus.trackingStatus?has_content>
                      <#assign descs = delegator.findByAnd("Enumeration", Static["org.ofbiz.base.util.UtilMisc"].toMap("enumCode", trackingStatus.trackingStatus), null, false)>
                      <#list descs as desc>
                        <br />
                        Delivery status: <b>${desc.description}</b>
                      </#list>
                    </#if>
                    </div>
                  </#if>
          ...
          <tr>
            ...
            <td width="5">&nbsp;</td>
            <td valign="top" width="80%">
                    <form name="getEpacketTrackingNumber" method="post" action="<@ofbizUrl>getEpacketTrackingNumber</@ofbizUrl>">
                        <input type="hidden" name="orderId" value="${orderId}"/>
                    </form>
                    <a href="javascript:document.getEpacketTrackingNumber.submit()" class="buttontext">Get ePacket Tracking Number</a>
            </td>
            </tr>
        </#if>
