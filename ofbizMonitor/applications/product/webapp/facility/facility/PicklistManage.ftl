  ...
  <div class="screenlet-body">
    <#if picklistInfoList?has_content>
      <#list picklistInfoList as picklistInfo>
        ...
        <#assign shippingMethod = picklistInfo.shipmentMethodType.getString("shipmentMethodTypeId")>
        <#-- Picklist -->
        <div>
          <span class="label">${uiLabelMap.ProductPickList} ${picklist.picklistId}
          <#if picklistInfo.shipmentMethodType?has_content>
            || ${picklistInfo.shipmentMethodType.description?default(picklistInfo.shipmentMethodType.shipmentMethodTypeId)}</span>
          </#if>
          </span>
          ...
          <#if shippingMethod == "EXPRESS" || shippingMethod == "EMS" || shippingMethod == "YW_EXPRESS" || shippingMethod == "EPACKET">
            <a href="<@ofbizUrl>expressPicklist?picklistId=${picklist.picklistId}</@ofbizUrl>" onclick="window.open(this.href,'','resizable=1,scrollbars=yes,height=480,width=640');return false;" class="buttontext">${uiLabelMap.Print} ${picklistInfo.shipmentMethodType.description?default(picklistInfo.shipmentMethodType.shipmentMethodTypeId)} ${uiLabelMap.OrderOrderId}</a>
          <#else>
            <a href="<@ofbizUrl>BellyannaShippingLabel.pdf?picklistId=${picklist.picklistId}</@ofbizUrl>" target="_blank" class="buttontext">${uiLabelMap.Print} ${uiLabelMap.ShippingLabel}</a>
          </#if>
          <#if shippingMethod == "EXPRESS" || shippingMethod == "EMS" || shippingMethod == "YW_EXPRESS">
            <a href="<@ofbizUrl>EMSShippingLabel.pdf?picklistId=${picklist.picklistId}</@ofbizUrl>" target="_blank" class="buttontext">${uiLabelMap.Print} EMS ${uiLabelMap.ShippingLabel}</a>
          </#if>
          <#if shippingMethod == "EU_SMALL_PARCEL" || shippingMethod == "YW_EXPRESS">
            <a href="<@ofbizUrl>YWBulkImportData.pdf?picklistId=${picklist.picklistId}</@ofbizUrl>" target="_blank" class="buttontext">${uiLabelMap.Print} ${uiLabelMap.YWBulkImportData}</a>
          </#if>
          <span class="label">|| ${uiLabelMap.ProductCreatedModifiedBy} ${picklist.createdByUserLogin}/${picklist.lastModifiedByUserLogin} ||</span>
          <#-- PicklistRole -->
        ...
        <#list picklistInfo.picklistRoleInfoList?if_exists as picklistRoleInfo>
          <div style="margin-left: 15px;">
            <span class="label">${uiLabelMap.ProductAssignPicker}</span> ${picklistRoleInfo.partyNameView.firstName?if_exists} ${picklistRoleInfo.partyNameView.middleName?if_exists} ${picklistRoleInfo.partyNameView.lastName?if_exists} ${picklistRoleInfo.partyNameView.groupName?if_exists} ||
             ${picklistRoleInfo.roleType.description}
        ...
        <hr />
        <#-- Bellyanna - START -->
        <div class="fieldgroup" id="${picklist.picklistId}">
          <div class="fieldgroup-title-bar">
            <ul>
              <li class="collapsed"><a onclick="javascript:toggleCollapsiblePanel(this, '${picklist.picklistId}_body', 'Expand', 'Collapse');">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Detailed Bins Information for Picklist ${picklist.picklistId}</a>
              </li>
            </ul>
          </div>
		  <div id="${picklist.picklistId}_body" class="fieldgroup-body" style="display: none;">
        <#-- Bellyanna - END -->
        <#-- PicklistBin -->
                ...
                <table class="basic-table" cellspacing="0">
                  ...
                  <#list picklistBinInfo.picklistItemInfoList?if_exists as picklistItemInfo>
                    ...
                    <tr valign="middle"<#if alt_row> class="alternate-row"</#if>>
                      <td><a href="/ordermgr/control/orderview?orderId=${picklistItem.orderId}" target="_blank">${picklistItem.orderId}</a></td>
