...
productFacilityLocations = delegator.findByAnd("ProductFacilityLocation", UtilMisc.toMap("facilityId", facilityId), null, false);
freeList = [];
productFacilityLocations.each { thisLocation ->
    freeList.add(thisLocation.locationSeqId);
}

freeLocations00GV = delegator.findList("FacilityLocation", EntityCondition.makeCondition(UtilMisc.toList(
                                                                                            EntityCondition.makeCondition("facilityId", EntityOperator.EQUALS, facilityId),
                                                                                            EntityCondition.makeCondition("areaId", EntityOperator.EQUALS, "00")//,
                                                                                            //EntityCondition.makeCondition("locationSeqId", EntityOperator.NOT_IN, freeList)
                                                                                            )
                                                                                    ,EntityOperator.AND),
                                    null, UtilMisc.toList("locationSeqId"), null, false);
freeLocations00 = [];
freeLocations00GV.each { this00GV ->
    freeLocations00.add(this00GV.locationSeqId);
}
freeLocations00.removeAll(freeList);
context.freeLocations00 = freeLocations00;

freeLocations01GV = delegator.findList("FacilityLocation", EntityCondition.makeCondition(UtilMisc.toList(
                                                                                            EntityCondition.makeCondition("facilityId", EntityOperator.EQUALS, facilityId),
                                                                                            EntityCondition.makeCondition("areaId", EntityOperator.EQUALS, "01")//,
                                                                                            //EntityCondition.makeCondition("locationSeqId", EntityOperator.NOT_IN, freeList)
                                                                                            )
                                                                                    ,EntityOperator.AND),
                                    null, UtilMisc.toList("locationSeqId"), null, false);
freeLocations01 = [];
freeLocations01GV.each { this01GV ->
    freeLocations01.add(this01GV.locationSeqId);
}
freeLocations01.removeAll(freeList);
context.freeLocations01 = freeLocations01;