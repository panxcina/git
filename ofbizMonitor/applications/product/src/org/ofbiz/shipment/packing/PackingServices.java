...
public class PackingServices {
    ...
    public static Map<String, Object> completePack(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        session.setWeightUomId(weightUomId);
        setSessionPackageWeights(session, packageWeights); Map<String, String> packageTrackingCodes = (Map) context.get("packageTrackingCodes"); setSessionPackageTrackingCodes(session, packageTrackingCodes);
    ...
    private static void setSessionPackageTrackingCodes(PackingSession session, Map<String, String> packageTrackingCodes) {
        if (! UtilValidate.isEmpty(packageTrackingCodes)) {
            Iterator<String> ptcit = packageTrackingCodes.keySet().iterator();
            while (ptcit.hasNext()) {
                String packageSeqId = (String) ptcit.next();
                String packageTrackingCode = (String) packageTrackingCodes.get(packageSeqId);
