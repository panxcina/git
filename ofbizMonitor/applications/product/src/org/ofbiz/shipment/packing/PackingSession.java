...
public class PackingSession implements java.io.Serializable {
    ...
    protected int status = 1; protected Map<String, String> packageTrackingCodes = null;
    ...
    public PackingSession(LocalDispatcher dispatcher, GenericValue userLogin, String facilityId, String binId, String orderId, String shipGrp) {
        ...
        this.packageSeq = 1;
        this.packageWeights = FastMap.newInstance(); this.packageTrackingCodes = new HashMap<String, String>();
    }
    ...
    public void clear() {
        ...
        this.packageSeq = 1;
        this.status = 1; if (this.packageTrackingCodes != null) this.packageTrackingCodes.clear();
    ...
    public String complete(boolean force) throws GeneralException {
        ...
        this.runEvents(PackingEvent.EVENT_CODE_COMPLETE); this.updateShipmentPackageRouteSegments();//save tracking codes
    ...
    protected void updateShipmentPackageRouteSegments() throws GeneralException {
        List shipmentPackageRouteSegments = getDelegator().findByAnd("ShipmentPackageRouteSeg", UtilMisc.toMap("shipmentId", this.getShipmentId()), null, false);
        if (! UtilValidate.isEmpty(shipmentPackageRouteSegments)) {
            Iterator sprit = shipmentPackageRouteSegments.iterator();
            while (sprit.hasNext()) {
                GenericValue shipmentPackageRouteSegment = (GenericValue) sprit.next();
                if (UtilValidate.isEmpty(shipmentPackageRouteSegment.get("shipmentPackageSeqId"))) continue;
    ...
    public String getPackageTrackingCode(String packageSeqId) {
        if (this.packageTrackingCodes == null) return null;
        if (! this.packageTrackingCodes.containsKey(packageSeqId)) return null;
        return (String) this.packageTrackingCodes.get(packageSeqId);
    }
